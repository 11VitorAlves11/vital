...
The development Compose stack deliberately enables a synthetic demo account at
`demo@example.com` / `VitalDemo2026!`. It is recreated when missing and includes longitudinal
lab results, body measurements, interventions and a scheduled repeat. This seed is disabled
by default everywhere else, and the API refuses `SEED_DEMO_DATA=true` in production.
